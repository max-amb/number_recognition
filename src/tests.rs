...
pub mod test {
    ...
    fn create_nn_for_test() -> (NN, DVector<f32>, DVector<f32>) {
        // Creating network
        let mut network = NN::new(&[3, 2, 3], InitialisationOptions::Random, Some(0.2));

        // Setting weights and biases
        network.weights = vec![
            DMatrix::from_vec(2, 3, vec![-0.75, 0.25, 0.5, -0.5, -0.25, 0.75]),
            DMatrix::from_vec(3, 2, vec![-0.75, 0.5, -0.25, 0.75, -0.5, 0.25]),
        ];
        network.biases = vec![
            DVector::from_vec(vec![0.0, 1.0]),
            DVector::from_vec(vec![0.75, 0.5, 0.25]),
        ];
    ...
    fn test_backpropagation() {
        ...
        let expected_output = [
            DVector::from_vec(vec![-0.0733288, 0.366644]),
            DVector::from_vec(vec![0.208924, -0.272186, 0.295432]),
        ];
        ...
        let expected_output = [
            DMatrix::from_vec(
                ...
                3,
                vec![
                    -0.0183322, 0.091661, -0.0366644, 0.183322, -0.0549966, 0.274983,
                ],
            ),
            DMatrix::from_vec(
                3,
                2,
                vec![
                    -0.0052231, 0.00680465, -0.0073858, 0.287271, -0.374256, 0.406219,
                ],
            ),
        ];
        dbg!(&delta_weights);
    ...
    fn test_forward_pass() {
        ...
        assert_eq!(
            NN::forward_pass(&network, &data, &CostFunction::Quadratic)[1],
            DVector::from_vec(vec![-0.025, 1.375])
        );
        assert_eq!(
            NN::forward_pass(&network, &data, &CostFunction::Quadratic)[2],
