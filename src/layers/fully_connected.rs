...
impl Forward for FullyConnected {
    fn run(&self, prev_layer: Ten) -> Ten {
        ...
        Ten { data: data.data, shape: self.output_shape }
    }
}

impl Initialisable for FullyConnected {
    fn initialise(&mut self, previous_shape: Shape) -> Shape {
        self.weights = Some(self.initialisation_options.create_matrix((self.output_shape.magnitude(), previous_shape.magnitude(), 1).into(), previous_shape.magnitude()));
