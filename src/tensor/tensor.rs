...
impl std::ops::Mul for &Ten {
    ...
    fn mul(self, rhs: &Ten) -> Self::Output {
        ...
        for i in 0..self.shape.channels {
            let self_view= self.data.view((i*(self_size),0), (self_size,1)).reshape_generic(Dyn(self.shape.nrows), Dyn(self.shape.ncols));
            let rhs_view= rhs.data.view((i*rhs_size,0), (rhs_size,1)).reshape_generic(Dyn(rhs.shape.nrows), Dyn(rhs.shape.ncols));
